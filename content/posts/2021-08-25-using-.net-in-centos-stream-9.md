...
Unfortunately, RHEL 9 (or CentOS
Stream 9, rather) is still in development. It has also switched to
OpenSSL 3.0, which currently-released versions of .NET don't support.
Even forcing OpenSSL 1.1 leads to an obscure `error:0E07607:
configuration file routines` error.

In the rest of this post, I will walk you through on how to get a
simple .NET Core 3.1 network-using application to work on CentOS
Stream 9. I will be using container images, but only because it makes
things easy to reproduce. There's nothing here specific to container
images.

# CentOS Stream? Why not RHEL 9?

CentOS Stream 9 is the in-development version of what will eventually
become RHEL 9. You can [find more details
here](https://www.redhat.com/en/blog/faq-centos-stream-updates). I am
going to use CentOS Stream 9 in the rest of this post.

First, let's get our hands on a container we can use for playing
around with. I am going to use the latest CentOS Stream 9 image:
...
I will use `podman` through this post, but feel free to replace with
`docker` and things should work more-or-less the same.
...
1. Use the CentOS Stream container image
...
If we look for all `.cnf` files installed by `compat-openssl11`, we
can find where a usable configuration file is:
